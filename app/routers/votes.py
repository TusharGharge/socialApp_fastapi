...
def create_votes(
    ...
    # if post with id does not exits
    post = db.query(models.Post).filter(models.Post.id == models.Vote.post_id).first()
    if not post:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"post with this id {vote.post_id} doesn't found",
        )
    ...
    if vote.dir == 1:
        if found_vote:
            raise HTTPException(
                ...
                detail=f"user{current_user.id} has been alredy voted on post {vote.post_id}",
            )
